...
Reproduce Figure 5.4 on page 180 from ISLR. Obviously since there is some randomness in the $k$-folds cross validation procedure you will not be able to do an exact replication of the right-hand pane of the plot. However, the LOOCV plot should be the same. Be sure to have your code hidden in your HTML as usual … I’ll be looking at it this time.
```{r}
rm(list=ls())
library(ISLR)
LOOCV<-c()
...
for (i in 1:10){
  LOOCV_mse<-c()
  for (j in 1:nrow(Auto)){
    df<-Auto[-j,]
    model<-lm(mpg ~ poly(horsepower,i), data=df)
    #dynamic memory allocation
    LOOCV_mse[j]<-(Auto$mpg[j]-predict.lm(model,Auto[j,]))^2
    LOOCV[i]<-mean(LOOCV_mse)
  }


  folds<-createFolds(1:nrow(Auto),k=10,returnTrain=TRUE)
  fold10_mse<-c()
  for (j in 1:10){
    AutoFold<-Auto[folds[[j]],]
    model<-glm(mpg ~ poly(horsepower,i), data=AutoFold)
    #dynamic memory allocation
    fold10_mse[j] <- mean( (Auto$mpg[-folds[[j]]] - 
                           predict.lm(model, Auto[-folds[[j]],]))^2)
    fold10[i]<-mean(fold10_mse)
  }
}

ggplot()+
  geom_path(aes(x=1:10,y=LOOCV))

ggplot()+
  geom_path(aes(x=1:10,y=fold10),color="red")
```
